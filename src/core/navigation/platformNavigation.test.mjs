...
test('platform home is the first canonical breadcrumb', () => {
  for (const path of ['/apps/inventory/unique-units', '/apps/crm/leads/123']) {
    assert.equal(getCanonicalBreadcrumbs(path, { currentLabel: '#123' })[0].to, '/app');
  }
});

test('Paperwork breadcrumbs start at the application and express its two-section IA', () => {
...
test('shared platform home never uses browser history', () => {
  ...
  const appShell = read('../../app/layouts/AppLayout.jsx');
  const crmShell = read('../../features/crm/layouts/CrmWorkspaceLayout.jsx');
  const showroomShell = read('../../features/showroom/layouts/ShowroomWorkspaceLayout.jsx');
  assert.match(home, /to=\{ROUTES\.app\}/);
  for (const source of [appShell, shell, crmShell, showroomShell]) assert.match(source, /PlatformHomeLink/);
  assert.doesNotMatch(`${home}\n${shell}`, /navigate\(-1\)|history\.back/);
});
