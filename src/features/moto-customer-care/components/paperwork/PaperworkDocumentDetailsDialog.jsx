...
import { Ban, Clock3, ExternalLink, FileImage, FileText, MapPin, TriangleAlert, UserRound, X } from 'lucide-react';
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent
        ...
        className="inset-0 z-[150] h-[100dvh] w-full max-w-none border-0 bg-slate-50 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:h-[min(88vh,900px)] sm:w-[min(94vw,1100px)] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-[30px] sm:border sm:border-slate-200"
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(4.5rem,calc(env(safe-area-inset-top)+4rem))] sm:p-6 lg:overflow-hidden">
          ...
          {isOwner && displayedDocument.status === 'in_custody' && !isLoading ? (
            <div className="mx-auto mt-5 max-w-[1020px] border-t border-slate-200 pt-4">
