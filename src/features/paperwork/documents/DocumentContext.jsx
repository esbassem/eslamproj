...
export function DocumentContext({ tenantId, requestId, returnLabel = 'المستند' }) {
  ...
  return <div className="space-y-3 text-sm"><div className="flex justify-between"><span className="font-bold text-slate-500">الطلب المرتبط</span><Link to={PAPERWORK_ROUTES.requestDetails(requestId)} state={createPaperworkNavigationState(location, { returnLabel })} className="rounded-lg px-2 py-1 font-black text-blue-700 hover:bg-blue-50">طلب #{requestId.slice(0, 8).toUpperCase()}</Link></div><div className="flex justify-between"><span className="font-bold text-slate-500">مرجع البيع</span><strong>{query.data?.saleNumber || '—'}</strong></div></div>;
}
