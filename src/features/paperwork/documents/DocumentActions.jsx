...
import { getPaperworkErrorMessage, PAPERWORK_PERMISSIONS } from '@/features/paperwork/authorization/paperworkPermissions';
...
export function DocumentActions({ document, tenantId, onChanged }) {
  const { can } = useAuthorization();
  const [menuOpen, setMenuOpen] = useState(false); const [action, setAction] = useState(null);
  const actions = can(PAPERWORK_PERMISSIONS.CORRECT) ? [{ id: 'owner', label: labels.owner }] : [];
  if (document.status === 'in_custody') {
    if (can(PAPERWORK_PERMISSIONS.AUTHORIZE_RELEASE)) actions.unshift({ id: 'release', label: labels.release });
    if (can(PAPERWORK_PERMISSIONS.CORRECT)) actions.unshift({ id: 'previous', label: labels.previous });
    if (can(PAPERWORK_PERMISSIONS.CANCEL)) actions.unshift({ id: 'cancel', label: labels.cancel });
  }
