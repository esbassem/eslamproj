...
export function PaperworkHomePage() {
  ...
  return <PaperworkPage title="إدارة أوراق الملكية" description="نقطة بدء تشغيلية سريعة دون تحميل قوائم النطاق كاملة." actions={<button type="button" onClick={() => setReceiptOpen(true)} className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-black text-white hover:bg-blue-700"><FilePlus2 className="h-4 w-4" />استلام ورق جديد</button>}>
    ...
    {receiptOpen ? <Suspense fallback={null}><ManualReceipt open={receiptOpen} onOpenChange={setReceiptOpen} tenantId={tenantId} userId={tenantUser?.id} onSaved={() => { setReceiptOpen(false); void query.retry(); }} /></Suspense> : null}
  </PaperworkPage>;
}
