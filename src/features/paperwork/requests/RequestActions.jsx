...
const actionLabels = {
  ...
  processor_cancellation: 'تأكيد إلغاء الطلب لدى الجهة', notify: 'تسجيل إخطار العميل', cancel: 'إلغاء الطلب',
...
function ActionDialog({ action, request, tenantId, onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (action === 'notify') await paperworkService.notifyPaperworkCustomer({ tenantId, requestId: request.id, channel: 'phone', notes });
...
export function RequestActions({ request, tenantId, isOwner = false, onChanged }) {
  const [action, setAction] = useState(null);
  const [menuOpen, setMenuOpen] = useState(false);
  const primary = getRequestPrimaryAction(request);
  const exceptional = !isOwner ? [{ id: 'notify', label: 'تسجيل إخطار العميل' }] : request.status === 'cancelled' ? [{ id: 'reopen', label: 'إعادة فتح الطلب' }] : [
    { id: 'notify', label: 'تسجيل إخطار العميل' }, { id: 'previous', label: 'تسجيل تسليم سابق' },
    { id: 'link', label: 'ربط مستند من الخزنة' }, { id: 'cancel', label: 'إلغاء الطلب' },
  ];
  if (action === 'receive') {
  ...
  return <div className="relative flex gap-2">{primary ? <button type="button" disabled={primary.disabled} onClick={() => !primary.disabled && setAction(primary.id)} className="h-10 rounded-xl bg-blue-600 px-4 text-sm font-black text-white disabled:bg-emerald-600">{primary.label}</button> : null}<button type="button" onClick={() => setMenuOpen((value) => !value)} className="grid h-10 w-10 place-items-center rounded-xl border bg-white" aria-label="المزيد من الإجراءات" aria-expanded={menuOpen}><MoreHorizontal className="h-5 w-5" /></button>{menuOpen ? <div className="absolute left-0 top-12 z-20 w-64 rounded-xl border bg-white p-1 shadow-xl">{exceptional.map((item) => <button key={item.id} type="button" onClick={() => { setMenuOpen(false); setAction(item.id); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-right text-sm font-black hover:bg-slate-50">{item.id === 'notify' ? <Bell className="h-4 w-4" /> : null}{item.label}</button>)}</div> : null}{action ? <ActionDialog action={action} request={request} tenantId={tenantId} onClose={() => setAction(null)} onDone={onChanged} /> : null}</div>;
}
