...
function DestinationDetailsSheet({ destination, tenantId, canManage, onClose, onChanged }) {
...
export function MoneyDestinationsSettings({ tenantId, canManage }) {
  ...
  const load = useCallback(async () => { setState((current) => ({ ...current, status: 'loading', error: '' })); try { const items = await moneyDestinationsService.list(tenantId); setState({ status: 'ready', items, error: '' }); } catch (error) { setState({ status: 'error', items: [], error: error.message }); } }, [tenantId]);
