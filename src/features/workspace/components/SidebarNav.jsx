...
function containsActiveHref(item, activeHref) {
  ...
  return normalizeActivePath(item.href) === activeHref
    || item.children.some((child) => containsActiveHref(child, activeHref));
}

function SidebarLink({ item, activeHref, depth = 0, hasChildren = false, isExpanded = false, isBranchActive = false, onToggle }) {
  const { t } = useI18n();
  ...
  const content = (
    <>
      <span className="flex min-w-0 flex-1 items-center gap-3">
        <span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition', depth > 0 && 'h-8 w-8', isActive ? 'bg-slate-200/70 text-slate-700' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200/70', isDisabled && 'bg-slate-100 text-slate-400 group-hover:bg-slate-100')}>
  ...
  if (hasChildren) {
    return (
      <div className={cn('group flex w-full items-center rounded-xl transition-colors', isActive || isBranchActive ? 'bg-slate-100/80 text-slate-950' : 'bg-transparent text-slate-700 hover:bg-slate-100/70')} dir="rtl">
        <NavLink
          to={item.href}
          end
          ...
          className={cn('flex min-w-0 flex-1 items-center gap-3 rounded-xl px-3 py-2.5 text-right', depth > 0 && 'py-2')}
        >
          {content}
        </NavLink>
        <button
          type="button"
          onClick={onToggle}
          className="ml-2 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-200/70 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-200"
          aria-label={isExpanded ? `طي ${label}` : `توسيع ${label}`}
          aria-expanded={isExpanded}
        >
          <ChevronDown className={cn('h-4 w-4 transition-transform duration-200', !isExpanded && '-rotate-90')} aria-hidden="true" />
        </button>
      </div>
    );
  }
  ...
  return (
    <NavLink to={item.href} end data-permission-key={item.permissionKey || undefined} onMouseEnter={() => preloadProtectedRoute(item.href)} onFocus={() => preloadProtectedRoute(item.href)} onTouchStart={() => preloadProtectedRoute(item.href)} className={cn('group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-right transition-colors', depth > 0 && 'py-2', isActive && 'bg-slate-100/80 text-slate-950', !isActive && 'bg-transparent text-slate-700 hover:bg-slate-100/70')} dir="rtl">
      {content}
    </NavLink>
  );
}
...
function SidebarMenuItem({ item, activeHref, depth }) {
  ...
  const isBranchActive = hasChildren && containsActiveHref(item, activeHref);
  const [isExpanded, setIsExpanded] = useState(isBranchActive);

  useEffect(() => {
    if (isBranchActive) setIsExpanded(true);
  }, [isBranchActive]);

  return (
    <div className="space-y-1">
      <SidebarLink item={item} activeHref={activeHref} depth={depth} hasChildren={hasChildren} isExpanded={isExpanded} isBranchActive={isBranchActive} onToggle={() => setIsExpanded((expanded) => !expanded)} />
      {hasChildren ? (
