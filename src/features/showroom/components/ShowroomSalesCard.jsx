...
import {
  ...
  useEmployeeCashDestination,
} from "@/features/showroom/components/EmployeeCashDestinationNotice";
...
function getPartnerAvatarConfig(partner) {
  const displayConfig =
    partner?.displayConfig ?? partner?.display_config ?? null;
  const avatar =
    displayConfig?.avatar && typeof displayConfig.avatar === "object"
      ? displayConfig.avatar
      : {};
  const text =
    String(avatar.text || partner?.name || "-")
      .trim()
      .slice(0, 1) || "-";
  ...
  return {
    text,
    bg:
      typeof avatar.bg === "string" && avatar.bg.trim() ? avatar.bg : "#0F172A",
    color:
...
function getProductTitle(product) {
  return (
    product?.displayName || product?.name || product?.description || "منتج"
  );
}
...
function getProductAttributeFields(product) {
  const saleFields = Array.isArray(product?.attributeFields)
  ...
  const rawAttributes = Array.isArray(product?.attributesJsonb)
    ? product.attributesJsonb
    : [];
...
function getIdentifierSlots(definition) {
  const schemaSlots =
    definition?.inputSchema?.slots ??
    definition?.input_schema?.slots ??
    definition?.slots ??
    [];
...
function validateIdentifierValue(definition, value) {
  ...
  if (
    ...
    !safeValue &&
    !(definition.allowNotAvailable && definition.isNotAvailable)
  ) {
  ...
  const invalidIndex = Array.from(safeValue).findIndex(
    (character, index) =>
      !isAllowedIdentifierCharacter(character, slots[index]?.type),
  );
  if (invalidIndex >= 0) {
...
export function ShowroomSalesCard({
  onSaleCreated,
  onSalePending,
  onPendingSaleDelete,
  resumeSale,
  ...
  const [isProductCategoriesLoading, setIsProductCategoriesLoading] =
    useState(false);
  const [productsError, setProductsError] = useState("");
  const [isProductCreateSheetOpen, setIsProductCreateSheetOpen] =
    useState(false);
  ...
  const [trackingIdentifierDefinitions, setTrackingIdentifierDefinitions] =
    useState([]);
  const [isTrackingIdentifiersLoading, setIsTrackingIdentifiersLoading] =
    useState(false);
  ...
  useEffect(() => {
    if (!crmContext?.lead) return;
    setCustomerName(crmContext.lead.customer_name || "");
    setProductSearch(crmContext.lead.interested_product_name || "");
    const financing = crmContext.requested_sale_type
      ? crmContext.requested_sale_type === "financing"
      : crmContext.lead.purchase_type === "installment" ||
        crmContext.lead.status === "installment_approved";
    setPaymentType(financing ? PAYMENT_TYPES.FINANCING : PAYMENT_TYPES.CASH);
    setFinancierPartnerId(
      crmContext.selected_approval?.company_partner_id || "",
    );
    setFinancedAmount(
      String(crmContext.selected_approval?.approved_finance_amount || ""),
    );
    setApprovalReference(crmContext.selected_approval?.company_reference || "");
  }, [crmContext]);
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      const nextProducts = await posService.listSellProducts({
        tenantId: tenant.id,
      });
  ...
  useEffect(() => {
    ...
    const resumedCustomer =
      resumeSale.customer ||
      (resumeSale.customer_id
        ? {
        ...
        : null);
    const resumedCart = (
      Array.isArray(resumeSale.lines) ? resumeSale.lines : []
    ).map((line) => ({
      ...line,
      id: line.id,
    ...
    Promise.all([
      ...
      .then(([categories, attributes, links]) => {
        ...
        const normalizedLinks = [...links].sort((left, right) => {
          ...
          return String(
            attributesById.get(left.attributeId)?.name ?? "",
          ).localeCompare(
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) =>
      `${product.name} ${product.displayName ?? ""} ${product.code} ${product.barcode ?? ""}`
        .toLowerCase()
        .includes(query),
    );
  ...
  const openCreditAmount = Object.values(openCreditAllocations).reduce(
  ...
  const pendingAttributeFields = useMemo(
    () => getProductAttributeFields(pendingProduct),
    [pendingProduct],
  );
    ...
    productCategoryTrackingIdentifierService
      ...
      .then((definitions) => {
        ...
        setProductDraft((current) => ({
          ...current,
          trackingIdentifiers: Object.fromEntries(
            (definitions ?? []).map((definition) => [
              definition.identifierTypeId,
              current.trackingIdentifiers?.[definition.identifierTypeId] ?? "",
            ]),
          ),
          trackingIdentifierNotAvailable: Object.fromEntries(
            (definitions ?? []).map((definition) => [
              definition.identifierTypeId,
  ...
  const addProduct = (product) => {
    setCart((current) => {
      ...
      const existing = current.find(
        (item) => (item.lineId || item.id) === productLineId,
      );
      if (existing) {
        return current.map((item) =>
          (item.lineId || item.id) === productLineId
  ...
  const updateQuantity = (lineId, nextQuantity) => {
    if (nextQuantity <= 0) {
      setCart((current) =>
        current.filter((item) => (item.lineId || item.id) !== lineId),
      );
      return;
    }
    setCart((current) =>
  ...
  const openProductSheet = (product) => {
    const attributes = Object.fromEntries(
      getProductAttributeFields(product).map((field) => [field.key, ""]),
    );
  ...
  const handleProductCreate = async (payload) => {
    ...
    try {
      ...
      const createdProductId =
        createdProduct.defaultProductProductId || createdProduct.id;
      const sellableProduct =
        nextProducts.find((product) => product.id === createdProductId) ??
        nextProducts.find(
          (product) => product.productTemplateId === createdProduct.id,
        ) ??
        null;
  ...
  const buildProductSelection = ({
    ...
    const price = validatePrice
      ...
      : Number(productDraft.price || pendingProduct.price || 0);
    ...
    if (pendingProduct.tracking === "serial") {
      for (const definition of trackingIdentifierDefinitions) {
        const value = String(
          productDraft.trackingIdentifiers?.[definition.identifierTypeId] ?? "",
        ).trim();
        const isNotAvailable = Boolean(
          productDraft.trackingIdentifierNotAvailable?.[
            definition.identifierTypeId
          ],
        );
        const validation = validateIdentifierValue(
          { ...definition, isNotAvailable },
          value,
        );
        if (validation !== true) {
    ...
    const missingRequiredAttribute = pendingAttributeFields.find(
      (field) =>
        field.isRequired &&
        !String(productDraft.attributes[field.key] || "").trim(),
    );
    if (missingRequiredAttribute) {
      setProductSheetError(
        `حدد ${missingRequiredAttribute.label} قبل إضافة المنتج.`,
      );
      return null;
    }
    ...
    const configuredAttributes = pendingAttributeFields
      .map((field) => {
        const draftValue = String(
          productDraft.attributes[field.key] || "",
        ).trim();
        const selectedValue = field.values?.find(
          (value) => value.id === draftValue,
        );
    ...
    const fallbackSerialNumber =
      pendingProduct.tracking === "serial" && !trackingIdentifiers.length
    ...
    if (includeLicense && pendingProduct.requiresLicense) {
      ...
      if (license.status === "licensed" && !license.expiresAt) {
        setProductSheetError(
          'أدخل تاريخ انتهاء الترخيص عندما تكون الحالة "مرخص".',
        );
        return null;
      }
    }
      ...
      {isLicenseFieldsLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block">
            <span className="mb-2.5 block text-xs font-black uppercase tracking-wide text-slate-500">
              رقم الرخصة
            </span>
            <Input
  ...
  const complete = async () => {
    ...
    try {
      const result = await onSaleCreated?.({
        ...
        financierPartnerId:
          paymentType === PAYMENT_TYPES.FINANCING ? financierPartnerId : null,
        approvalReference:
          paymentType === PAYMENT_TYPES.FINANCING
            ? approvalReference.trim()
            : "",
        paymentNotes:
          paymentType === PAYMENT_TYPES.FINANCING ? paymentNotes.trim() : "",
      });
  ...
  const deletePendingSale = async () => {
    ...
    if (
      !window.confirm(
        "سيتم حذف الفاتورة المعلقة وكل بياناتها نهائيًا. هل أنت متأكد؟",
      )
    )
      return;
    setMessage("");
  ...
  return (
    <section className="flex h-[min(620px,calc(100vh-9rem))] flex-col bg-white text-[#173653]">
      ...
      <div
        className={`flex-1 bg-white ${step === 1 ? "overflow-hidden" : "overflow-y-auto px-5 py-4 sm:px-7"}`}
      >
        <div
          className={`mx-auto w-full ${step === 1 ? "h-full max-w-none" : "max-w-[620px]"}`}
        >
          {step === 0 && (
          ...
          {step === 1 && (
            <div className="h-full w-full">
              <div className="grid h-full md:grid-cols-[1fr_300px]" dir="ltr">
                ...
                <div
                  className="flex min-h-0 flex-col overflow-hidden border-l border-slate-200 bg-slate-50"
                  dir="rtl"
                >
                    ...
                    {isProductsLoading ? (
                      ...
                      filteredProducts.map((product) => {
                        ...
                        return (
                          ...
                          >
                            <span className="min-w-0">
                              <span className="block truncate text-sm font-black text-[#173653]">
                                {getProductTitle(product)}
                              </span>
                              <span className="mt-0.5 block truncate text-xs font-bold text-[#8aa0b3]">
                                {productCode}
                              </span>
                              {product.tracking === "serial" ? (
                                <span className="mt-1 block text-[11px] font-black text-[#668097]">
                                  سيتم إدخال تعريفات التتبع عند البيع
                                </span>
                              ) : null}
                            </span>
                            <span className="rounded-lg bg-slate-900 px-3 py-1.5 text-sm font-black text-white">
                      ...
                      <div className="m-4 rounded-2xl border border-dashed border-[#e6c8cf] bg-[#fcf6f7] px-5 py-7 text-center shadow-[0_22px_50px_-36px_rgba(78,24,35,0.35)]">
                        ...
                        <p className="mt-4 text-sm font-black text-[#6f4e56]">
                          لا توجد منتجات مطابقة.
                        </p>
                        <p className="mt-2 text-xs font-bold leading-6 text-[#9a7a82]">
                          {productSearch.trim()
                            ? `يمكنك إضافة "${productSearch.trim()}" مباشرة ثم متابعة البيع.`
                            : "أضف منتجًا جديدًا ثم ارجع لإكمال عملية البيع."}
                        </p>
                        <Button
                          type="button"
                        ...
                        >
                          ...
                          {productSearch.trim() && !hasExactProductMatch
                            ? `إضافة المنتج "${productSearch.trim()}"`
                            : "إضافة منتج جديد"}
                        </Button>
                      </div>
                    )}
          ...
          {step === 2 && (
            <SaleReviewStep customer={customer} cart={cart} total={total} />
          )}

          {step === 3 && (
            <div
              className="flex h-full w-full items-start justify-center px-6 py-7"
              dir="rtl"
            >
                ...
                <div className="grid gap-8 border-b border-slate-200 pb-7 lg:grid-cols-[0.8fr_1.2fr]">
                  ...
                  <section>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      ...
                      {!isOpenCreditsLoading && !openCreditsError
                        ? openCredits.map((credit) => {
                            const avatar = getPartnerAvatarConfig({
                              name: credit.paymentEntityName,
                              displayConfig: credit.displayConfig,
                            });
                            const selectedAmount =
                              Number(
                                openCreditAllocations[credit.openCreditLineId],
                              ) || 0;
                            return (
                              <button
                                key={credit.openCreditLineId}
                                type="button"
                                onClick={() => setIsOpenCreditsSheetOpen(true)}
                                className={`flex min-h-16 items-center gap-3 border px-3 py-2 text-right transition focus:outline-none ${selectedAmount > 0 ? "border-emerald-300 bg-emerald-50" : "border-slate-200 hover:border-slate-400"}`}
                              >
                                <span
                                  className={`flex h-9 w-9 shrink-0 items-center justify-center text-xs font-black ${avatar.shape === "circle" ? "rounded-full" : "rounded-lg"}`}
                                  style={{
                    ...
                    {openCredits
                      ...
                      .map((credit) => {
                        ...
                        return (
                          <div
                            key={credit.openCreditLineId}
                            className="flex w-full min-w-0 items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2"
                          >
                            <span
                              className={`flex h-7 w-7 shrink-0 items-center justify-center text-[10px] font-black ${avatar.shape === "circle" ? "rounded-full" : "rounded-md"}`}
                              style={{
                                backgroundColor: avatar.bg,
                                color: avatar.color,
                              }}
                            ...
                            <span className="shrink-0 truncate text-xs font-black text-slate-700">
                              {credit.paymentEntityName}
                            </span>
                            <span className="min-w-0 flex-1 truncate text-xs font-bold text-slate-400">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                setOpenCreditAllocations((current) => ({
                                  ...current,
                                  [credit.openCreditLineId]: "",
                                }))
                              }
                              className="flex h-7 w-7 shrink-0 items-center justify-center text-slate-400 transition hover:bg-red-50 hover:text-red-600"
                              aria-label={`حذف اعتماد ${credit.paymentEntityName}`}
                              title="حذف الاعتماد"
                            >
                ...
                <div className="mt-7 flex items-end justify-between gap-4 border-t-2 border-slate-950 pt-5">
                  <div>
                    ...
                    <p className="mt-2 text-xs font-bold text-slate-400">
                      {hasNoRemainingAmount
                        ? "تم سداد المبلغ بالكامل"
                        : "يظل مستحقًا بعد إتمام البيع"}
                    </p>
                  </div>
                  <p
                    className={`text-3xl font-black tracking-tight ${hasNoRemainingAmount ? "text-emerald-600" : "text-[#9b3645]"}`}
                  >
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          <div className="flex gap-2">
            {step === 3 && pendingSaleId ? (
              <Button
                variant="secondary"
                onClick={deletePendingSale}
                disabled={isDeletingPendingSale || isCompleting}
                className="h-11 rounded-xl border-red-200 bg-white px-4 font-black text-red-600 hover:bg-red-50"
              >
            ...
            <Button
              variant="secondary"
              onClick={reset}
              disabled={isCompleting || isDeletingPendingSale}
              className="h-11 rounded-xl border-slate-200 bg-white px-5 font-black text-slate-800 hover:bg-slate-50"
            >
              مسح
            </Button>
            {step > 0 && (
            ...
            {step < 2 ? (
              <Button
                onClick={next}
                className="h-11 rounded-xl bg-slate-900 px-7 font-black text-white shadow-[0_16px_24px_-18px_rgba(15,23,42,0.78)] hover:bg-slate-800"
              >
                التالي
              ...
              <Button
                ...
                disabled={isCreatingPendingSale}
                className="h-11 rounded-xl bg-slate-900 px-7 font-black text-white shadow-[0_16px_24px_-18px_rgba(15,23,42,0.78)] hover:bg-slate-800"
              >
      ...
      <Sheet
        open={isCashPaymentSheetOpen}
        onOpenChange={setIsCashPaymentSheetOpen}
      >
        <SheetContent
          side="right"
          className="w-full max-w-md border-l-0 bg-white p-0"
          dir="rtl"
        >
            ...
            <div className="space-y-5">
              <label className="block">
                ...
                <Input
                  ...
                  inputMode="decimal"
                  min="0"
                  max={Math.max(total - openCreditAmount, 0)}
                  step="0.01"
                  value={cashDraftAmount}
                  onChange={(event) => {
                      ...
                      value === ""
                        ...
                        : String(
                            Math.min(
                              ...
                              Math.max(total - openCreditAmount, 0),
                            ),
                          ),
                    );
                  }}
                  placeholder="0"
                  className="h-14 rounded-xl border-slate-300 text-xl font-black text-slate-950"
                  autoFocus
                />
                <span className="mt-2 block text-xs font-bold text-slate-400">
                  الحد الأقصى{" "}
                  {formatMoney(Math.max(total - openCreditAmount, 0))}
                </span>
              </label>
              <label className="block">
                <span className="mb-2 block text-xs font-black text-slate-600">
                  ملاحظة الدفع <span className="text-red-600">*</span>
                </span>
                <Input
                  required
                  value={cashDraftNote}
                  onChange={(event) => setCashDraftNote(event.target.value)}
                  placeholder="مثال: دفعة نقدية من العميل"
                  className="h-12 rounded-xl border-slate-300 text-sm font-bold"
                />
              </label>
              <EmployeeCashDestinationNotice
                destination={cashDestination}
          ...
          <div className="mt-auto flex gap-3 border-t border-slate-200 px-5 py-4">
            <Button
              ...
              disabled={
                (Number(cashDraftAmount) || 0) <= 0 ||
                !cashDraftNote.trim() ||
                isCashDestinationLoading ||
                Boolean(cashDestinationError) ||
                !cashDestination?.accountId
              }
              className="h-11 flex-1 rounded-xl bg-slate-950 font-black text-white hover:bg-slate-800"
            >
              تأكيد الدفع
            </Button>
            {Number(paidAmount) > 0 ? (
              <Button
                ...
                className="h-11 rounded-xl font-black text-red-600"
              >
                حذف
              </Button>
            ) : null}
      ...
      <Sheet
        open={isOpenCreditsSheetOpen}
        onOpenChange={setIsOpenCreditsSheetOpen}
      >
        <SheetContent
          side="right"
          className="w-full max-w-md border-l-0 bg-[#fffafb] p-0"
          dir="rtl"
        >
          <SheetHeader className="border-b border-[#e6c8cf] bg-white px-5 py-5 text-right">
            <div className="flex items-center justify-between gap-3">
              <div>
                <SheetTitle className="text-xl font-black text-[#4d1f28]">
                  اعتمادات العميل المفتوحة
                </SheetTitle>
                <p className="mt-1 text-xs font-bold text-[#8e6f76]">
                  اختر كل اعتماد مستقل والمبلغ المستخدم منه
                </p>
              </div>
              <SheetDismissButton />
            </div>
          ...
          <SheetBody className="space-y-3 px-5 py-5">
            {openCredits.map((credit) => {
              ...
              return (
                <div
                  key={credit.openCreditLineId}
                  className="rounded-2xl border border-[#ead9dd] bg-white p-4"
                >
                  <div className="mb-3 flex min-w-0 items-center gap-3">
                    <span
                      className={`flex h-11 w-11 shrink-0 items-center justify-center text-sm font-black ${avatar.shape === "circle" ? "rounded-full" : "rounded-xl"}`}
                      style={{
                        backgroundColor: avatar.bg,
                        color: avatar.color,
                      }}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-black text-[#4d1f28]">
                        {credit.paymentEntityName}
                      </p>
                      <p className="mt-0.5 text-[11px] font-bold text-[#8e6f76]">
                        أصلي {formatMoney(credit.originalAmount)} · مستخدم{" "}
                        {formatMoney(credit.reconciledAmount)}
                      </p>
                      <p className="mt-0.5 text-xs font-bold text-emerald-700">
                        المتاح: {formatMoney(credit.availableAmount)}
                      </p>
                      <p className="mt-0.5 text-[11px] font-bold text-slate-400">
                        {credit.moveDate
                          ? new Date(credit.moveDate).toLocaleDateString(
                              "ar-EG",
                            )
                          : "بدون تاريخ"}
                  ...
                  <label className="block text-xs font-black text-[#668097]">
                    ...
                    <Input
                      ...
                      onChange={(event) => {
                        ...
                        setOpenCreditAllocations((current) => ({
                          ...current,
                          [credit.openCreditLineId]: safeValue,
                        }));
                      }}
            ...
            <div className="sticky bottom-0 rounded-2xl bg-slate-950 p-4 text-white shadow-lg">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-lg font-black">
                  {formatMoney(openCreditAmount)}
                </span>
              </div>
              <Button
                type="button"
                onClick={() => setIsOpenCreditsSheetOpen(false)}
                className="mt-3 h-11 w-full rounded-xl bg-white font-black text-slate-950 hover:bg-slate-100"
              >
                تأكيد
              </Button>
            </div>
      ...
      <Sheet open={productSheetOpen} onOpenChange={closeProductSheet}>
        <SheetContent
          side="bottom"
          className="mx-auto max-h-[96vh] min-h-[68vh] w-full max-w-[680px] rounded-t-[28px] border-0 bg-white shadow-none"
          dir="rtl"
        >
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0 flex-1">
                <SheetTitle className="text-xl font-black tracking-tight text-white">
                  {getProductTitle(pendingProduct)}
                </SheetTitle>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <p className="text-xs font-bold text-white/75">
                    {getProductCode(pendingProduct)}
                  </p>
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <Button
                  ...
                  className="h-10 rounded-lg bg-white px-6 font-black text-slate-900 shadow-[0_8px_16px_-12px_rgba(15,23,42,0.5)] hover:bg-slate-100"
                >
                  {pendingProduct?.requiresLicense && productSheetStep === 1
                    ? "التالي"
                    : "إضافة"}
                </Button>
              </div>
                      ...
                      <div className="grid gap-4">
                        <label className="block">
                          ...
                          <span className="mt-2 block text-[10px] font-semibold text-slate-400">
                            السعر المقترح:{" "}
                            {formatMoney(Number(pendingProduct?.price ?? 0))}
                          </span>
                        </label>
                      </div>
                    ) : null}
                          ...
                          {trackingIdentifierDefinitions.map((definition) => {
                            ...
                            return (
                              ...
                              >
                                  ...
                                  {definition.allowNotAvailable ? (
                                    <span className="flex items-center gap-1.5 text-slate-500">
                                      <input
                                        ...
                                        onChange={(event) =>
                                          handleTrackingIdentifierNotAvailableChange(
                                            definition,
                                ...
                                <Input
                                  value={
                                    productDraft.trackingIdentifiers?.[
                                      definition.identifierTypeId
                                    ] ?? ""
                                  }
                                  inputMode={
                                  ...
                                  onChange={(event) =>
                                    handleTrackingIdentifierChange(
                                      definition,
                                      event.target.value,
                                    )
                                  }
...
function SaleReviewStep({
  ...
  return (
    <div className="mx-auto w-full max-w-[680px] space-y-3" dir="rtl">
      <p className="text-right text-xs font-black text-red-600">
        مراجعة الفاتورة قبل الدفع
      </p>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="grid gap-3 px-4 py-3 text-sm font-bold text-slate-700 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-1 font-mono text-slate-950" dir="ltr">
          ...
          {showPaymentSummary ? (
            <>
              <ReviewAmount
                label="المدفوع"
                value={paidAmount}
                note={paymentMethod?.trim() || "غير محددة"}
              />
              <ReviewAmount
                label="المتبقي"
...
function ReviewAmount({ label, value, tone = "default", note = "" }) {
  ...
  return (
    <div className="rounded-xl bg-slate-50 px-3 py-2">
      ...
      <p className={`mt-1 font-mono text-sm font-black ${toneClass}`}>
        {formatMoney(value)}
      </p>
      {note ? (
...
function AttributeSelectCard({ field, value, isOpen, onToggle, onSelect }) {
  ...
  const displayValue =
    selectedValue?.name || (field.isRequired ? "اختر القيمة" : "بدون اختيار");
      ...
      >
        <span className="flex items-start justify-between gap-3">
          <span className="min-w-0">
            ...
            <span
              className={`mt-1 block truncate text-sm font-black ${selectedValue ? "text-slate-950" : "text-slate-400"}`}
            >
              {displayValue}
            </span>
          </span>
          <span
            className={`mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition ${isOpen ? "rotate-180 bg-slate-900 text-white" : "group-hover:bg-white"}`}
          >
...
function SelectedProductsPanel({ cart, onUpdateQuantity, customer }) {
  return (
    <div
      className="flex h-full min-h-0 flex-col overflow-hidden bg-white px-5 py-4"
      dir="rtl"
    >
      {/* Customer card */}
      {customer && (
        <div className="mb-4 flex shrink-0 items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-black text-[#173653]">
              {customer.name}
            </p>
            {customer.phone && (
              <p className="text-xs font-bold text-[#668097]">
                {customer.phone}
              </p>
            )}
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-black text-slate-900">
                      {formatMoney(item.price * item.quantity)}
                    </span>
...
function LineMeta({ item, className = "" }) {
  const attributesText = item.configuredAttributes?.length
    ? item.configuredAttributes
        .map((attribute) => `${attribute.label}: ${attribute.value}`)
        .join(" - ")
    : "";
  const trackingText = item.trackingIdentifiers?.length
    ? item.trackingIdentifiers
        .map((identifier) => `${identifier.label}: ${identifier.value}`)
        .join(" - ")
    : "";
  const licenseText = item.license?.number
    ? `ترخيص: ${item.license.number}`
    : item.license
      ? "يحتاج ترخيص"
      : "";
  ...
  return (
    <div className={`space-y-0.5 text-[11px] font-bold leading-5 ${className}`}>
      {trackingText ? (
        <p className="truncate">{trackingText}</p>
      ) : item.serialNumber ? (
        <p className="truncate">السيريال: {item.serialNumber}</p>
      ) : null}
    ...
    <div className={`flex items-center ${compact ? "gap-1" : "gap-2"}`}>
      <button
        ...
        className={`${compact ? "h-8 w-8 rounded-lg" : "h-9 w-9 rounded-xl"} flex items-center justify-center border transition ${
          isLight
            ? "border-white/20 bg-white/14 text-white hover:bg-white/22"
            : "border-[#f0c9c9] bg-[#fff5f5] text-[#b43b3b] hover:bg-[#ffe9e9]"
        }`}
        ...
        <button
          ...
          className={`${compact ? "h-8 w-8 rounded-lg" : "h-9 w-9 rounded-xl"} flex items-center justify-center border transition ${
            isLight
              ...
              : "border-[#b8e0cf] bg-[#ecfbf4] text-[#167450] hover:bg-[#dff7ec]"
          }`}
