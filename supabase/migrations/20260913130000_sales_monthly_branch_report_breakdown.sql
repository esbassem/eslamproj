...
begin
  ...
  monthly_sales as materialized (
    select sale.id, sale.branch_id, sale.created_by,
    ...
    where sale.tenant_id = v_tenant_id and sale.status = 'confirmed'
      and sale.effective_sale_date >= v_month_start and sale.effective_sale_date < v_month_end
  )
...
comment on function public.get_sales_monthly_branch_reports(date) is
  'Monthly Sales totals, outstanding balances, and branch-scoped salesperson breakdowns.';
