-- Keep the invoice partner and the name printed on the document as two
-- independent values when receiving paperwork from the processor drawer.
...
begin
  ...
  if v_document.id is null then
    insert into public.paperwork_documents (
      tenant_id, branch_id, paperwork_request_id, tracking_unit_id,
      owner_partner_id, document_owner_name, source_type, document_type,
      document_title, status, notes, created_by
    ) values (
      p_tenant_id, v_request.branch_id, v_request.id, v_request.tracking_unit_id,
    ...
    set owner_partner_id = v_request.customer_id,
