...
begin
  ...
  if v_cash > 0 then
    select count(*) into v_account_count from public.account_accounts where tenant_id=v_sale.tenant_id and code='111002' and active=true;
    if v_account_count <> 1 then raise exception 'حساب 111002 غير موجود مرة واحدة داخل الشركة.'; end if;
    select id into v_cash_account from public.account_accounts where tenant_id=v_sale.tenant_id and code='111002' and active=true;
  end if;
  ...
  if v_cash > 0 then
    ...
    insert into public.account_move_lines (id,tenant_id,move_id,account_id,partner_id,label,quantity,unit_price,debit,credit,line_type,is_reconciled,amount_residual,amount_residual_currency,parent_state,currency_code,created_by)
    values
      (gen_random_uuid(),v_sale.tenant_id,v_cash_move_id,v_cash_account,null,'تحصيل نقدي لفاتورة شو روم',1,v_cash,v_cash,0,'liquidity',true,0,0,'posted','EGP',v_user.id),
