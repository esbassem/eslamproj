...
begin
 ...
 insert into public.res_users_groups(tenant_id,user_id,group_id) select tid,x.id,p_group_id from unnest(ids)x(id)
 on conflict on constraint res_users_groups_tenant_user_group_key do nothing;
