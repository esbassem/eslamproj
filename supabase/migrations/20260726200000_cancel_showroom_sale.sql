  ...
  if coalesce(v_user.role, '') not in ('owner', 'admin', 'accountant')
     and not exists (
       ...
       where membership.tenant_id = p_tenant_id
         ...
         and (permission_group.tenant_id is null
              or permission_group.tenant_id = p_tenant_id)
     )
  then
    raise exception 'إلغاء الفواتير المرحلة متاح للمالك أو الإدارة أو المحاسب فقط.';
  end if;
...
comment on function public.cancel_showroom_sale(uuid, uuid, text, text) is
  'Atomically cancels a confirmed showroom sale after locked preview revalidation.';
