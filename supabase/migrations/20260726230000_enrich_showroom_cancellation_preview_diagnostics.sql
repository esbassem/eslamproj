...
begin
    ...
    if v_move.id is null then
      ...
      if v_move.state <> 'posted' then
        v_failures := v_failures || jsonb_build_array(jsonb_build_object(
          ...
          'details', jsonb_build_object(
            ...
            'move_state', v_move.state,
            'move_reference', v_move.ref
          )
        ));
      end if;
      ...
      if v_move_debit <= 0 or v_move_debit <> v_move_credit then
        v_failures := v_failures || jsonb_build_array(jsonb_build_object(
          ...
          'details', jsonb_build_object(
            ...
            'difference', round(v_move_debit - v_move_credit, 2)
          )
        ));
      end if;
      ...
      if v_existing_reversal is not null then
        v_failures := v_failures || jsonb_build_array(jsonb_build_object(
          ...
          'details', jsonb_build_object(
            'move_name', v_existing_reversal ->> 'name',
            'move_state', v_existing_reversal ->> 'state',
            'move_date', v_existing_reversal ->> 'date'
          )
        ));
        v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
  ...
  if v_receivable_account_count <> 1 then
    v_failures := v_failures || jsonb_build_array(jsonb_build_object(
      ...
      'details', jsonb_build_object(
        'matching_account_count', v_receivable_account_count
      )
    ));
  ...
  if v_move.id is not null and v_receivable_account_id is not null then
    ...
    if v_receivable_line_count <> 1 then
      v_failures := v_failures || jsonb_build_array(jsonb_build_object(
        ...
        'details', jsonb_build_object(
          'move_name', v_move.name,
          'receivable_line_count', v_receivable_line_count
        )
      ));
    else
      select move_line.*
      ...
      if v_invoice_line.debit <= 0 or v_invoice_line.credit <> 0 then
        v_failures := v_failures || jsonb_build_array(jsonb_build_object(
          ...
          'details', jsonb_build_object(
            'move_name', v_move.name,
            'debit_amount', round(v_invoice_line.debit, 2),
            'credit_amount', round(v_invoice_line.credit, 2)
          )
        ));
      end if;
      ...
      if round(v_invoice_line.debit, 2) <> round(v_sale.total_amount, 2) then
        v_failures := v_failures || jsonb_build_array(jsonb_build_object(
          ...
          'details', jsonb_build_object(
            'invoice_total', round(v_sale.total_amount, 2),
            'accounting_receivable_amount', round(v_invoice_line.debit, 2),
            'difference', round(v_sale.total_amount - v_invoice_line.debit, 2)
          )
        ));
      end if;
      ...
      for v_row in
        select
          ...
          source_move.date as source_move_date,
          ...
          credit_line.label as source_line_description
        from public.account_partial_reconcile reconcile
        ...
        if v_allocation_blocker then
          v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
            ...
            'details', jsonb_strip_nulls(jsonb_build_object(
              'classification', v_classification,
              'legacy_kind', case
                when v_counterpart_count = 1 and v_counterpart.code = '119001'
                  then 'legacy_collection_119001'
                when lower(coalesce(v_row.pay_method, '')) = 'advance_credit'
                     or v_has_legacy_account
                  then 'legacy_advance_212001'
                else null
              end,
              'account_code', case
                when v_counterpart_count = 1 then v_counterpart.code
                else null
              end,
              'account_name', case
                when v_counterpart_count = 1 then v_counterpart.name
                else null
              end,
              'amount', round(v_row.amount, 2),
              'pay_method', v_row.pay_method,
              'source_move_name', v_row.source_move_name,
              'source_move_date', v_row.source_move_date,
              'description', v_row.source_line_description
            ))
          ));
        end if;
    ...
    if v_inventory_blocker is not null then
      ...
      v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
        ...
        'details', jsonb_strip_nulls(jsonb_build_object(
          ...
          'tracking_identifier', v_row.tracking_number,
          'tracking_type', v_row.tracking,
          'current_tracking_status', v_row.tracking_status,
          'expected_tracking_status', case
            when v_row.tracking = 'serial' then 'sold'
            else null
          end,
          'quantity', round(v_row.quantity, 2),
          'stock_out_quantity', v_stock_out_quantity,
          'reason', v_inventory_blocker
        ))
      ));
    end if;
  ...
  for v_row in
    select request.id, request.status, request.current_stage,
           request.created_at, request.updated_at, request.cancel_reason
  ...
  loop
    if v_row.current_stage = 'preparation' then
      ...
      v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
        ...
        'details', jsonb_build_object(
          'current_stage', v_row.current_stage,
          'current_stage_display', case v_row.current_stage
            when 'preparation' then 'مرحلة الإعداد'
            when 'owner_confirmation' then 'تحديد صاحب الأوراق'
            when 'sent_to_processor' then 'تم الإرسال إلى الجهة'
            when 'processor_ready' then 'الأوراق جاهزة لدى الجهة'
            when 'received_from_processor' then 'تم استلام الأوراق من الجهة'
            when 'client_notified' then 'تم إبلاغ العميل'
            when 'delivered' then 'تم التسليم للعميل'
            when 'cancelled' then 'ملغي'
            else 'مرحلة غير معروفة'
          end,
          'request_status', v_row.status,
          'request_date', v_row.created_at
        )
      ));
    end if;
  end loop;
