...
begin select*into c from phase7_context;select*into r from phase7_resources;r.customer_move:=gen_random_uuid();
 ...
 insert into public.account_moves(id,tenant_id,journal_id,name,move_type,partner_id,invoice_date,date,amount_total,state,ref,currency_code,created_by)values(r.customer_move,c.tenant_id,c.journal_id,'P7 CUSTOMER RECEIPT','journal',r.customer,current_date-17,current_date-17,20000,'posted','phase7_customer_payment','EGP',c.owner_id);
 ...
 insert into public.account_moves(id,tenant_id,journal_id,name,move_type,partner_id,invoice_date,date,amount_total,state,ref,currency_code,created_by)values(invoice_move,c.tenant_id,c.journal_id,'P7 INVOICE','journal',r.customer,current_date-12,current_date-12,15000,'posted','phase7_invoice','EGP',c.owner_id);
...
begin select*into c from phase7_context;select*into r from phase7_resources;
 ...
 -- A historical invoice remains allocatable prospectively in the current open period.
 x:=public.allocate_financial_payment(c.tenant_id,r.customer_payment,r.customer_target,5000,'p7-allocation',current_date);allocation:=(x->>'allocation_id')::uuid;
 ...
 -- Closed backdating is rejected atomically, while a current open-period allocation/unallocation remains valid.
 perform public.set_financial_period_lock(c.tenant_id,current_date-1,true,'Phase10B3A close prior periods');
 begin perform public.allocate_financial_payment(c.tenant_id,r.customer_payment,r.customer_target,1000,'p7-closed-allocation',current_date-7);raise exception'CLOSED_BACKDATED_ALLOCATION_ACCEPTED';exception when check_violation then null;end;
 if exists(select 1 from public.financial_payment_allocations where tenant_id=c.tenant_id and idempotency_key='p7-closed-allocation')then raise exception'CLOSED_ALLOCATION_RECORD_PERSISTED';end if;
 x:=public.allocate_financial_payment(c.tenant_id,r.customer_payment,r.customer_target,1000,'p7-open-allocation',current_date);allocation:=(x->>'allocation_id')::uuid;
 perform public.unallocate_financial_payment_allocation(c.tenant_id,allocation,'Open-period unallocation');
 -- Once an allocation's own effective period is closed, destructive unallocation is rejected.
 perform public.set_financial_period_lock(c.tenant_id,current_date-1,false,'Prepare historical allocation fixture');
 x:=public.allocate_financial_payment(c.tenant_id,r.customer_payment,r.customer_target,1000,'p7-later-closed-allocation',current_date-1);allocation:=(x->>'allocation_id')::uuid;
 perform public.set_financial_period_lock(c.tenant_id,current_date-1,true,'Close allocation effective period');
 begin perform public.unallocate_financial_payment_allocation(c.tenant_id,allocation,'Must remain closed');raise exception'CLOSED_ALLOCATION_UNALLOCATED';exception when check_violation then null;end;
 if not exists(select 1 from public.financial_payment_allocations where id=allocation and status='active')or not exists(select 1 from public.account_partial_reconcile p join public.financial_payment_allocations a on a.partial_reconcile_id=p.id where a.id=allocation)then raise exception'CLOSED_ALLOCATION_STATE_MUTATED';end if;
 perform public.set_financial_period_lock(c.tenant_id,current_date-1,false,'Release test lock');
 perform public.unallocate_financial_payment_allocation(c.tenant_id,allocation,'Fixture cleanup after unlock');
 begin perform public.allocate_financial_payment(c.tenant_id,r.customer_payment,r.customer_target,1000,'p7-future-allocation',current_date+1);raise exception'FUTURE_ALLOCATION_ACCEPTED';exception when check_violation then null;end;
 perform public.configure_financial_posting_policy(c.tenant_id,true,'Permit explicit future allocation proof');x:=public.allocate_financial_payment(c.tenant_id,r.customer_payment,r.customer_target,1000,'p7-future-allocation-allowed',current_date+1);allocation:=(x->>'allocation_id')::uuid;perform public.unallocate_financial_payment_allocation(c.tenant_id,allocation,'Future override proof cleanup');perform public.configure_financial_posting_policy(c.tenant_id,false,'Restore default future policy');
 perform public.set_financial_period_lock(c.tenant_id,current_date-1,true,'Close original accounting period before reversal proof');
 ...
 begin perform public.reverse_financial_payment_accounting(c.tenant_id,r.customer_payment,'closed date','p7-reverse-bad-date',current_date-1);raise exception'CLOSED_PERIOD_REVERSAL_ACCEPTED';exception when check_violation then null;end;
